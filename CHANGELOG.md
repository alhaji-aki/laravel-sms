# Changelog

All notable changes to `laravel-sms` will be documented in this file.

## 0.3.1 - 2026-04-13

### What's Changed

* update safe packages by @alhaji-aki in https://github.com/alhaji-aki/laravel-sms/pull/14

**Full Changelog**: https://github.com/alhaji-aki/laravel-sms/compare/0.3.0...0.3.1

## 0.3.0 - 2026-04-12

### What's Changed

* Bump dependabot/fetch-metadata from 2.3.0 to 2.4.0 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/5
* Bump stefanzweifel/git-auto-commit-action from 5 to 6 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/6
* Bump actions/checkout from 4 to 5 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/7
* Bump stefanzweifel/git-auto-commit-action from 6 to 7 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/8
* Bump dependabot/fetch-metadata from 2.4.0 to 2.5.0 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/10
* Bump dependabot/fetch-metadata from 2.5.0 to 3.0.0 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/12
* Bump ramsey/composer-install from 3 to 4 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/11
* Add support for PHP 8.5 and Laravel 13 by @alhaji-aki in https://github.com/alhaji-aki/laravel-sms/pull/13
* Bump actions/checkout from 5 to 6 by @dependabot[bot] in https://github.com/alhaji-aki/laravel-sms/pull/9

**Full Changelog**: https://github.com/alhaji-aki/laravel-sms/compare/0.2.0...0.3.0

## 0.2.0 - 2025-04-21

### What's Changed

* Bump dependabot/fetch-metadata from 2.2.0 to 2.3.0 by @dependabot in https://github.com/alhaji-aki/laravel-sms/pull/3
* Laravel 12 Support by @alhaji-aki in https://github.com/alhaji-aki/laravel-sms/pull/4

**Full Changelog**: https://github.com/alhaji-aki/laravel-sms/compare/0.1.1...0.2.0

## 0.1.1 - 2024-09-18

**Full Changelog**: https://github.com/alhaji-aki/laravel-sms/compare/0.1.0...0.1.1

## 0.1.0 - 2024-09-18

### What's Changed

* added arkesel sender by @alhaji-aki in https://github.com/alhaji-aki/laravel-sms/pull/2

### New Contributors

* @alhaji-aki made their first contribution in https://github.com/alhaji-aki/laravel-sms/pull/2

**Full Changelog**: https://github.com/alhaji-aki/laravel-sms/compare/0.0.1...0.1.0

## Initial Release - 2024-07-23

### What's Changed

* Bump dependabot/fetch-metadata from 2.1.0 to 2.2.0 by @dependabot in https://github.com/alhaji-aki/laravel-sms/pull/1

### New Contributors

* @dependabot made their first contribution in https://github.com/alhaji-aki/laravel-sms/pull/1

**Full Changelog**: https://github.com/alhaji-aki/laravel-sms/commits/0.0.1
